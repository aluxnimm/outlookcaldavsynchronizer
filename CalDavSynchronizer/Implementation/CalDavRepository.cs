  ...
  public class CalDavRepository<TContext> : IEntityRepository<WebResourceName, string, IICalendar, TContext>
  {
    ...
    private Task<IReadOnlyList<EntityWithId<WebResourceName, IICalendar>>> ParallelDeserialize(IReadOnlyList<EntityWithId<WebResourceName, string>> serializedEntities, ILoadEntityLogger logger)
    {
      return Task.Factory.StartNew(() =>
      {
        ...
        foreach (var serialized in serializedEntities)
        {
          IICalendar calendar;
          ...
          if (TryDeserializeCalendar(fixedICalData, out calendar, serialized.Id, _calendarSerializer, NullLoadEntityLogger.Instance))
          {
            ...
            {
              result.Add(EntityWithId.Create(serialized.Id, calendar));
            }
          }
          else
          {
            ...
            if (TryDeserializeCalendar(fixedICalData2, out calendar, serialized.Id, _calendarSerializer, logger))
            {
              // Add only if there is atleast one vevent or vtodo to avoid Nullreference Exceptions when processing
              if ((_entityType == CalDavRepository.EntityType.Event && calendar.Events.Count > 0) ||
                  (_entityType == CalDavRepository.EntityType.Todo && calendar.Todos.Count > 0))
